...
- `guide-kit` планировщик больше не отдаёт занятие без элемента каталога. Дефолтное узкое место RCS-профиля ведёт в область «Ограничения», где нет ни одной практики, а каталог мировоззренческих элементов пуст, если не задан `GUIDE_KIT_CURRICULUM_PATH` — то есть у каждого пользователя без рассчитанного профиля занятие собиралось без опоры на каталог, и модель придумывала его целиком. Добавлен кросс-областной запасной путь: берётся обслуживаемая область с наибольшим остаточным разрывом, поэтому реальный прогресс продолжает направлять выбор.
