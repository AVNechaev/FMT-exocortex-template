...
### 9. Запись итогов
**9a.** Дописать «Итоги дня» в DayPlan (шаблон: `memory/templates-dayplan.md`). Валидация: «Завтра начать с» непустое + каждый pending РП с конкретным next action. Postcondition: bash-grep по паттерну `Итоги дня|Day summary` (оба языка — issue #234: при `language: english` заголовок DayPlan «Day summary», русский grep всегда FAIL) → `9a OK/FAIL`. При архивации DayPlan (шаг 3) — frontmatter `status: active` → `status: closed` (единственный детерминированный сигнал «день закрыт», который читает day-open-pipeline.sh — не полагаться на текст commit message).
...
jq -n \
  --arg date "$(date +%Y-%m-%d)" \
  --arg commit "$(git -C "${IWE_GOVERNANCE_REPO:-$HOME/IWE/DS-strategy}" rev-parse HEAD)" \
