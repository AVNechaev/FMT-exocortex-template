...
**Карточка метода (`id` матчит `*.M.NNN`, любой Pack-домен) — доп. проверка (WP-448 Ф12, решение Б, действует с 2026-07-27):** тело кандидата обязано содержать заполненные разделы `## Forces` и `## Bias-Annotation` — не заголовок-плейсхолдер, а ≥1 строка таблицы с названной силой/искажением. Эталон формата: `SPF/pack-template/03-methods/_method-card-template.md`. Отсутствует раздел или он пуст → invalid, вернуть R15 на reject с паттерном «карточка метода без Forces/Bias-Annotation» ИЛИ (если R15 предпочитает не терять кандидата) на Шаг 3 для дозаполнения перед повторной проверкой. Это правило — только для карточек, создаваемых ЭТИМ проходом; ранее существующие карточки без этих разделов задним числом не инвалидируются (миграция бэклога — отдельный батч WP-448 Ф12, не эта проверка).
