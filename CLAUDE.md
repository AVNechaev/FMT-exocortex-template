...
**Пять правил inline (полный набор - в `memory/feedback_community_communication_style.md`):**
1. **Без служебных меток.** Никаких кодов форм, номеров рабочих продуктов, имён таблиц, воркеров, технических идентификаторов в тексте для человека.
2. **Английский - только после русского описания, в скобках.** Исключения: имена собственные и термины, которые сам участник употребляет. Жаргон запрещён.
3. **Главная мысль - в первых двух предложениях.** Второстепенное - ниже, под спойлером.
4. **Документы >2 экранов - спойлеры.** Каждая секция в `<details><summary><b>Название</b></summary>`. Без разделителей между спойлерами.
5. **Активный залог, машинные маркеры запрещены.** «Я нашёл», не «было обнаружено». «Прошло проверку», не «PASS».
