...
rg -Fq '**OwnerIntegrity** (HD #35): Один факт — один **авторитетный** владелец.' "$HOT" \
    || fail "hot-правило не ссылается на HD #35"
rg -Fq 'Неуправляемый дубль = ошибка синхронизации' "$HOT" \
    || fail "hot-правило не запрещает независимый дубль"
rg -Fq 'производная копия — только с автосинком' "$HOT" \
    || fail "hot-правило не определяет производную копию"
rg -Fq 'неполный пересказ — только с явной ссылкой на первоисточник' "$HOT" \
    || fail "hot-правило не определяет пересказ"

rg -Fq '## 35. Один авторитетный владелец ≠ одно буквальное место (OwnerIntegrity)' "$FULL" \
    || fail "развёрнутое различение #35 отсутствует"
for carrier in 'Авторитетный носитель' 'Производная копия' 'Неполный пересказ'; do
    rg -Fq "$carrier" "$FULL" || fail "нет класса носителя: $carrier"
done
rg -Fq 'сколько мест нужно исправить руками?' "$FULL" \
    || fail "операционный тест OwnerIntegrity отсутствует"
rg -Fq 'Неуправляемый дубль — не четвёртый класс носителя, а состояние нарушения' "$FULL" \
